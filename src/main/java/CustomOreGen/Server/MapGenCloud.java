...
public class MapGenCloud extends MapGenOreDistribution {
    ...
    @DistributionSetting(name = "CloudRadius", info = "Cloud horizontal radius, in meters")
    public final PDist clRadius = new PDist(25.0F, 10.0F);
    @DistributionSetting(name = "CloudThickness", info = "Cloud thickness (vertical radius), in meters")
    public final HeightScaledPDist clThickness = new HeightScaledPDist(14.0F, 6.0F);
    @DistributionSetting(name = "CloudSizeNoise", info = "Noise level added to cloud radius and thickness")
    public final PDist clSizeNoise = new PDist(0.2F, 0.0F);
    @DistributionSetting(name = "CloudHeight", info = "Height of cloud, in meters")
    public final HeightScaledPDist clHeight;
    @DistributionSetting(name = "CloudInclination", info = "Cloud angle from horizontal plane, in radians")
    ...
    private class DiffuseCloudComponent extends Component {
        ...
        public DiffuseCloudComponent(StructureGroup structureGroup, Transform transform, Random random) {
            ...
            float maxSize = (float) Math
                .max(super.boundingBox.getXSize(), Math.max(super.boundingBox.getYSize(), super.boundingBox.getZSize()))
                * 0.2F;
        ...
        public float getNoise(float x, float y, float z) {
            ...
            for (int i = 0; i < this.noiseLevels; ++i) {
                float im = (float) (1 << i);
                noise += (double) (1.0F / im)
                    * this.noiseGen.noise((double) (x * im), (double) (y * im), (double) (z * im));
            }
        ...
        public boolean addComponentParts(World world, Random random, StructureBoundingBox bounds) {
            if (this.invMat == null) {
                ...
                for (int x = Math.max(super.boundingBox.minX, bounds.minX); x
                    ...
                    for (int y = Math.min(super.boundingBox.maxY, bounds.maxY); y
                        >= Math.max(super.boundingBox.minY, bounds.minY); --y) {
                        for (int z = Math.max(super.boundingBox.minZ, bounds.minZ); z
                            ...
                            if (r2 <= maxNoisyR2) {
                                if (r2 > minNoisyR2) {
                                    ...
                                    if (r > 0.0F) {
                                        mult += this.sizeNoiseMagnitude
                                            * this.getNoise(pos[0] / r, pos[1] / r, pos[2] / r);
                                    } else {
                                ...
                                if (orVolumeNoiseCutoff.getMin() <= 1.0F
                                    && (orVolumeNoiseCutoff.getMax() <= 0.0F
                                        || (this.getNoise(pos[0], pos[1], pos[2]) + 1.0F) / 2.0F
                                            >= orVolumeNoiseCutoff.getValue(random))
