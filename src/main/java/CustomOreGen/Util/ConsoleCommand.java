...
public class ConsoleCommand extends CommandBase {
        ...
        if (text != null && recipient != null) {
            for (String line : text.split("\n")) {
                if (recipient instanceof EntityPlayerMP) {
                    (new CustomPacketPayload(PayloadType.CommandResponse, line))
                        .sendToClient((EntityPlayerMP) recipient);
                } else {
    ...
    public String getCommandName() {
        ...
        return cmdDef != null && cmdDef.names() != null && cmdDef.names().length > 0 ? cmdDef.names()[0]
            : this._method.getName();
    }

    public List<String> getCommandAliases() {
        CommandDelegate cmdDef = (CommandDelegate) this._method.getAnnotation(CommandDelegate.class);
        return cmdDef != null && cmdDef.names() != null && cmdDef.names().length > 1
            ? Arrays.asList(Arrays.copyOfRange(cmdDef.names(), 1, cmdDef.names().length))
            : null;
    }
        ...
        for (int cmdDef = 0; cmdDef < ptypes.length; ++cmdDef) {
            ...
            if (!clazz.isAssignableFrom(ICommandSender.class)) {
                if (clazz.isAssignableFrom(WorldServer.class)) {
                    if (getSenderWorld(sender) != null) continue;
            ...
            for (int pidx = 0; pidx < ptypes.length; ++pidx) {
                ...
                if (clazz.isAssignableFrom(ICommandSender.class)) {
                    ...
                    if (pvalues[pidx] == null) {
                        Integer dimId = ConfigParser
                            .parseString(Integer.class, ex < args.length ? args[ex++] : defValue);
                        pvalues[pidx] = dimId == null ? null
                            : MinecraftServer.getServer()
                                .worldServerForDimension(dimId.intValue());

                        if (pvalues[pidx] == null && required) {
                            throw new SyntaxErrorException(
                                "Missing or invalid dimension ID for required argument \'" + name + "\'",
                                new Object[0]);
                        }
                    }
