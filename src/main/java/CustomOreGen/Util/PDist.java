...
public class PDist implements Copyable<PDist> {
    ...
    private float inverseGaussian(double mu, double lambda, Random rand) {
        ...
        v *= v;
        double x = mu + (mu * mu * v) / (2 * lambda)
            - (mu / (2 * lambda)) * Math.sqrt(4 * mu * lambda * v + mu * mu * v * v);
        if (rand.nextDouble() <= (mu) / (mu + x)) // sample from a uniform distribution between 0 and 1
            return (float) x;
        else return (float) ((mu * mu) / x);
    }
    ...
    public String toString() {
        return String.format(
            "%f +- %f %s",
            new Object[] { Float.valueOf(this.mean), Float.valueOf(this.range), this.type.name() });
    }
}
