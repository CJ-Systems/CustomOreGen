...
public class SimpleProfiler {
    ...
    private int getSlot(Object key) {
        ...
        for (slot = Math.abs(key.hashCode()) % this._sections.length; this._sections[slot] != null
            && this._sections[slot].key != key; slot = (slot + 1) % this._sections.length) {
            ;
        }
    ...
    public void dumpSections(int sortBy) {
        ...
        while (it.hasNext()) {
            Section s = it.next();
            System.out.format(
                "%40s =  %8d  %12d (%8.0f) (%6.2f%%)  %12d (%8.0f) (%6.2f%%)\n",
                new Object[] { s.key, s.hits, s.runTime / 1000L, (float) s.runTime / 1000.0F / (float) s.hits,
                    100.0D * (double) s.runTime / maxOpentime, s.openTime / 1000L,
