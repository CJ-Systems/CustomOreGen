...
public class MapGenVeins extends MapGenOreDistribution {
    ...
    @DistributionSetting(name = "branchType", info = "Vein branch type (Bezier or Ellipsoid)")
    public BranchType brType;
    @DistributionSetting(name = "MotherlodeFrequency", info = "Number of motherlodes per 16x16 chunk")
    ...
    @DistributionSetting(name = "MotherlodeSize", info = "Motherlode size (radius), in meters")
    public final PDist mlSize;
    @DistributionSetting(name = "MotherlodeHeight", info = "Height of motherlode, in meters")
    public final HeightScaledPDist mlHeight;
    @DistributionSetting(name = "BranchFrequency", info = "Number of branches per motherlode")
    public final PDist brFrequency;
    @DistributionSetting(name = "BranchInclination", info = "Branch angle from horizontal plane, in radians")
    public final PDist brInclination;
    @DistributionSetting(name = "BranchLength", info = "Length of branches, in meters")
    ...
    @DistributionSetting(name = "SegmentForkFrequency", info = "Forking rate of each segment")
    public final PDist sgForkFrequency;
    @DistributionSetting(name = "SegmentForkLengthMult", info = "Multiplier to remaining branch length for each fork")
    public final PDist sgForkLenMult;
    @DistributionSetting(name = "SegmentLength", info = "Length of branch segments, in meters")
    ...
    @DistributionSetting(name = "SegmentRadius", info = "Cross-section radius of branch segments, in meters")
    public final PDist sgRadius;
    @DistributionSetting(name = "OreDensity", info = "Density multiplier for individual ore blocks")
    public final PDist orDensity;
    @DistributionSetting(name = "OreRadiusMult", info = "Radius multiplier for individual ore blocks")
    ...
    public Component generateStructure(StructureGroup structureGroup, Random random) {
        ...
        if (!structureGroup.canPlaceComponentAt(0, mlX, mlY, mlZ, random)) {
            ...
            for (int br = this.brFrequency.getIntValue(random); br > 0; --br) {
                ...
                this.generateBranch(
                    structureGroup,
                    this.brLength.getValue(brRandom),
                    maxHeight,
                    minHeight,
                    segMat,
                    motherlode,
    ...
    public void generateBranch(StructureGroup structureGroup, float length, float maxHeight, float minHeight,
        Transform mat, Component parent, Random random) {
        ...
        while (length > 0.0F) {
            ...
            Transform segMat = mat.clone()
                .scale(segRad, segRad, segLen);
            ...
            if (!structureGroup
                .canPlaceComponentAt(((Component) component).getComponentType() + 1, pos[0], pos[1], pos[2], random)) {
                return;
            }
            ...
            for (int axisTheta = this.sgForkFrequency.getIntValue(random); axisTheta > 0; --axisTheta) {
                ...
                fkMat.rotate(
                    this.sgAngle.getValue(fkRandom),
                    MathHelper.cos(axisTheta1),
                    MathHelper.sin(axisTheta1),
                    0.0F);
                float fkLenMult = this.sgForkLenMult.getValue(fkRandom);
                this.generateBranch(
                    structureGroup,
                    length * (fkLenMult > 1.0F ? 1.0F : fkLenMult),
                    maxHeight,
                    minHeight,
                    fkMat,
                    (Component) component,
    ...
    private class BezierTubeComponent extends Component {
        ...
        public BezierTubeComponent(StructureGroup structureGroup, Transform transform) {
            ...
            super.boundingBox = new StructureBoundingBox(
                MathHelper.floor_float(bb[0]),
                MathHelper.floor_float(bb[1]),
                MathHelper.floor_float(bb[2]),
                MathHelper.floor_float(bb[3]) + 1,
                MathHelper.floor_float(bb[4]) + 1,
                MathHelper.floor_float(bb[5]) + 1);
        ...
        public void setChild(Component comp) {
            ...
            if (this.next != null) {
                ...
                StructureBoundingBox bb = new StructureBoundingBox(
                    MathHelper.floor_float(pos[0] - rMax),
                    MathHelper.floor_float(pos[1] - rMax),
                    MathHelper.floor_float(pos[2] - rMax),
                    MathHelper.floor_float(pos[0] + rMax) + 1,
                    MathHelper.floor_float(pos[1] + rMax) + 1,
                    MathHelper.floor_float(pos[2] + rMax) + 1);
        ...
        public void interpolateDerivative(float[] der, float t) {
            if (t > 0.0F && this.next != null) {
                ...
                der[0] = 2.0F
                    * ((1.0F + t) * (this.mid[0] - this.prev.end[0]) - t * (this.prev.end[0] - this.prev.mid[0]));
                der[1] = 2.0F
                    * ((1.0F + t) * (this.mid[1] - this.prev.end[1]) - t * (this.prev.end[1] - this.prev.mid[1]));
                der[2] = 2.0F
                    * ((1.0F + t) * (this.mid[2] - this.prev.end[2]) - t * (this.prev.end[2] - this.prev.mid[2]));
            } else {
        ...
        public float interpolateRadius(float t) {
            return t > 0.0F && this.next != null ? (1.0F - t) * this.rad + t * this.next.rad
                : (t < 0.0F && this.prev != null ? (1.0F + t) * this.rad - t * this.prev.rad
                    : (t <= 0.0F && t > -1.0F ? this.rad
                        : (t > 0.0F && t < 0.5F ? this.rad * MathHelper.sqrt_float(1.0F - 4.0F * t * t) : 0.0F)));
        }
        ...
        public boolean addComponentParts(World world, Random random, StructureBoundingBox bounds) {
            ...
            do {
                ...
                if (this.context.radius > 0.0F) {
                    ...
                    boolean intersects = bb[3] >= (float) bounds.minX && bb[0] <= (float) bounds.maxX
                        && bb[5] >= (float) bounds.minZ
                        && bb[2] <= (float) bounds.maxZ
                        && bb[4] >= (float) bounds.minY
                        && bb[1] <= (float) bounds.maxY;
        ...
        class interpolationContext {
            ...
            public void init(float stepSize, boolean calculateDirection) {
                ...
                if (this.calcDer) {
                    ...
                    this.derLen = MathHelper
                        .sqrt_float(this.der[0] * this.der[0] + this.der[1] * this.der[1] + this.der[2] * this.der[2]);
            ...
            public boolean advance(float tolerance) {
                ...
                do {
                    ...
                    if (this.calcDer) {
                        ...
                        this.derLen = MathHelper.sqrt_float(
                            this.der[0] * this.der[0] + this.der[1] * this.der[1] + this.der[2] * this.der[2]);
                        if (derLen > 0.0F) {
                            this.der[0] /= this.derLen;
                            this.der[1] /= this.derLen;
                            this.der[2] /= this.derLen;
                    ...
                    countDeadlock--;
                    if (countDeadlock == 0) {
                        CustomOreGenBase.log.info(
                            String.format(
                                "Deadlock at pos (%f %f %f) der (%f %f %f) radius %f with tolerance %f",
                                pX,
                                pY,
                                pZ,
                                dX,
                    ...
                    if (countDeadlock <= 0) {
                        CustomOreGenBase.log.info(
                            String.format(
                                "Deadlock count is %d, in %s, maxErr %f, nt %f, delta(%f %f %f), d2 %f, der_delta(%f %f %f), derErr %f",
                                countDeadlock,
                                this,
                                maxErr,
                                nt,
                ...
                throw new RuntimeException(
                    "CustomOreGen: Detected a possible infinite loop during bezier interpolation.  Please report this error.");
            }

            @Override
            public String toString() {
                return String.format(
                    "BezierTubeComponent.interpolationContext[pos(%f %f %f) der(%f %f %f) derLen %f radius %f err %f t %f dt %f calcDer %s]",
                    pos[0],
                    pos[1],
                    pos[2],
                    der[0],
    ...
    class SolidSphereComponent extends Component {
        ...
        public boolean addComponentParts(World world, Random random, StructureBoundingBox bounds) {
            if (this.invMat == null) {
                ...
                for (int x = Math.max(super.boundingBox.minX, bounds.minX); x
                    <= Math.min(super.boundingBox.maxX, bounds.maxX); ++x) {
                    for (int y = Math.max(super.boundingBox.minY, bounds.minY); y
                        <= Math.min(super.boundingBox.maxY, bounds.maxY); ++y) {
                        for (int z = Math.max(super.boundingBox.minZ, bounds.minZ); z
                            <= Math.min(super.boundingBox.maxZ, bounds.maxZ); ++z) {
                            pos[0] = (float) x + 0.5F;
                            pos[1] = (float) y + 0.5F;
                            pos[2] = (float) z + 0.5F;
    ...
    public double getAverageOreCount() {
        ...
        double brVolume = VolumeHelper
            .cylindricalVolume(this.getAverageBranchLength(), this.sgRadius.mean * this.orRadiusMult.mean);
