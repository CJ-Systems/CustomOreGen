...
public class ValidatorDistribution extends ValidatorNode {
    ...
    protected boolean validateChildren() throws ParserException {
        try {
            this.distribution = this._distributionFactory
                .createDistribution(this.getParser().target.nextDistributionID());
        } catch (Exception var7) {
            throw new ParserException(
                "Failed to create distribution using \'" + this._distributionFactory + "\'.",
                this.getNode(),
                var7);
        }

        this.getNode()
            .setUserData("value", this.distribution, (UserDataHandler) null);
        ...
        if (inherits != null) {
            ...
            if (settings.isEmpty()) {
                throw new ParserException(
                    "Cannot inherit settings (\'" + inherits + "\' is not a loaded distribution).",
                    this.getNode());
            }

            if (settings.size() > 1) {
                throw new ParserException(
                    "Cannot inherit settings (\'" + inherits
                        + "\' is ambiguous; matching "
                        + settings.size()
        ...
        HashSet<String> settings1 = new HashSet<String>(
            this.distribution.getDistributionSettingDescriptions()
                .keySet());
        ...
        if (settings1.contains(nameKey)) {
            ...
            try {
                ...
                throw new ParserException(
                    "Attribute \'" + nameKey + "\' cannot be set (" + var9.getMessage() + ").",
                    this.getNode(),
                    var9);
            }
        ...
        if (settings1.contains(displayNameKey)) {
            ...
            try {
                ...
                throw new ParserException(
                    "Attribute \'" + displayNameKey + "\' is not configurable.",
                    this.getNode(),
                    e);
            } catch (IllegalArgumentException e) {
                throw new ParserException(
                    "Attribute \'" + displayNameKey + "\' cannot be set (" + e.getMessage() + ").",
                    this.getNode(),
                    e);
            }
    ...
    public void validateDistributionSettings(Set<String> settings) throws ParserException {
        ...
        if (settings.contains(parentKey)) {
            for (Node oreBlockKey = this.getNode()
                .getParentNode(); oreBlockKey != null; oreBlockKey = oreBlockKey.getParentNode()) {
                ...
                if (replacesKey != null && replacesKey instanceof IOreDistribution) {
                    ...
                    this.getNode()
                        .setUserData("validated", Boolean.valueOf(true), (UserDataHandler) null);
                    break;
                }
            }
        ...
        if (settings.contains(oreBlockKey)) {
            ...
            for (ValidatorBlockDescriptor oreBlock : validateNamedChildren(
                2,
                "OreBlock",
                new ValidatorBlockDescriptor.Factory())) {
            ...
            if (!oreBlockDesc.getDescriptors()
                .isEmpty()) {
                try {
                    ...
                    throw new ParserException(
                        "Target ore blocks are not supported by this distribution.",
                        this.getNode(),
                        var17);
                }
            }
        ...
        if (settings.contains(biomeKey)) {
            ...
            for (ValidatorBiomeDescriptor biome : validateNamedChildren(
                2,
                "Biome",
                new ValidatorBiomeDescriptor.Factory())) {
                biomeDescriptor.add(biome.biome, biome.weight, biome.restriction, false);
            }

            for (ValidatorBiomeDescriptor biomeType : validateNamedChildren(
                2,
                "BiomeType",
                new ValidatorBiomeDescriptor.Factory())) {
            ...
            if (!biomeDescriptor.getDescriptors()
                .isEmpty()) {
                try {
        ...
        for (String settingName : settings) {
            ...
            try {
                ...
                throw new ParserException(
                    "Attribute \'" + settingName + "\' is not configurable.",
                    this.getNode(),
                    var10);
            } catch (IllegalArgumentException var11) {
                throw new ParserException(
                    "Attribute \'" + settingName + "\' cannot be set (" + var11.getMessage() + ").",
                    this.getNode(),
                    var11);
            }
        }
    ...
    private void validatePlacementRestriction(Set<String> settings, StandardSettings setting) throws ParserException {
        ...
        if (settings.contains(settingKey)) {
            ...
            for (ValidatorBlockDescriptor replaces : validateNamedChildren(
                2,
                settingKey,
                new ValidatorBlockDescriptor.Factory())) {
            ...
            if (!replacesDesc.getDescriptors()
                .isEmpty()) {
                try {
                    ...
                    throw new ParserException(
                        "'" + settingKey + "' not supported by this distribution.",
                        this.getNode(),
                        var15);
                }
            }
