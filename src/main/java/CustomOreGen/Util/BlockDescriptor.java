...
public class BlockDescriptor implements Copyable<BlockDescriptor> {
    ...
    public static class BlockInfo {
        ...
        public int hashCode() {
            ...
            if (this.nbt != null) code ^= nbt.hashCode();
            return code;
        }
    ...
    public BlockDescriptor add(String descriptor, float weight, boolean describesOre, boolean matchFirst,
        boolean regexp, NBTTagCompound nbt) {
    ...
    private void add(Block block, int metadata, NBTTagCompound nbt, float weight) {
        if (nbt != null && !block.hasTileEntity(metadata)) {
            throw new IllegalArgumentException(
                "NBT specified, but matching block " + block.getUnlocalizedName()
                    + ":"
                    + metadata
                    + " lacks tile entity");
        }
        ...
        if (!this._compiled) {
            ...
            for (Descriptor desc : this._descriptors) {
                if (desc.describesOre) {
                    ...
                    if (block != null) this.add(block, desc.getMeta(), desc.nbt, desc.weight);
                }
            }
        }
    }
    ...
    private class Descriptor {
        ...
        public Descriptor(String description, float weight, boolean describesOre, boolean matchFirst, boolean regexp,
            NBTTagCompound nbt) {
        ...
        public int getMeta() {
            boolean hasMeta = this.getBlockName()
