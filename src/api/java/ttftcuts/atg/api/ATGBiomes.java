...
public abstract class ATGBiomes {
     ...
     *
     * @return an array of three doubles corresponding to the height, temperature and moisture at the specified point in
     *         the ranges 0.0-1.0.
     */
     ...
     *
     * @param salt
     *                  Biome blob generation salt. Used to offset biome boundaries from other groups to avoid strange
     *                  artifacts.
     *
     * @param generate
     *                  Set to false to prevent this group generating in the default manner. Primarily for use with the
     *                  biome group assignment events.
     */
    public static void addBiomeGroup(BiomeType type, String name, double temp, double moisture, double height,
        double minHeight, double maxHeight, long salt, boolean generate) {
        ATGBiomeGroupAddEvent event = new ATGBiomeGroupAddEvent(
            type,
            name,
            temp,
            moisture,
    ...
    public static void addBiomeGroup(BiomeType type, String name, double temp, double moisture, double height,
        double minHeight, double maxHeight, long salt) {
     ...
     * Best used to ensure a height-constrained biome group generates in favour of an otherwise identically ranged
     * group.
     *
     * @param type
     ...
     *
     * @param modifier
     *                 Modifier value. Positive makes the group more likely to be picked. Very small values can have a
     *                 large effect.
     */
    public static void modGroupSuitability(BiomeType type, String name, double modifier) {
        ATGBiomeGroupEvent event = new ATGBiomeGroupEvent(
            ATGBiomeGroupEvent.EventType.SUITABILITY,
            type,
            name,
            modifier);
        MinecraftForge.EVENT_BUS.post(event);
     ...
     */
    public static void replaceBiome(BiomeType type, String group, BiomeGenBase toReplace, BiomeGenBase replacement,
        double weight) {
     ...
     *
     * @param weight
     *                 Generation weight for the sub-biome. The parent biome is always weighted at 1.0, so a 1.0 weight
     *                 here with a single sub-biome would be a 50/50 split.
     */
    public static void addSubBiome(BiomeGenBase biome, BiomeGenBase subBiome, double weight) {
        ATGBiomeModEvent event = new ATGBiomeModEvent(
            ATGBiomeModEvent.EventType.SUBBIOME,
            biome,
            null,
            subBiome,
            weight);
    ...
    /**
     * Use this to enable the posting of "ATGBiomeGroupAssignmentEvent"s at generation, to allow custom biome group
     * overrides.
     * If this is not called at least once, none of those events will be sent.
     *
     * Listening for ATGBiomeGroupAssignmentEvent allows direct replacement of the biome group at every x/z coordinate
     * pair.
