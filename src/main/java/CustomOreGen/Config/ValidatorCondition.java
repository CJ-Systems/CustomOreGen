...
public abstract class ValidatorCondition extends ValidatorNode {
    ...
    protected boolean validateChildren() throws ParserException {
        ...
        for (Node condition = this.getNode()
            .getFirstChild(); condition != null; condition = condition.getNextSibling()) {
            if (condition.getNodeType() == 1) {
                if (condition.getNodeName()
                    .equalsIgnoreCase("Then")) {
                    trueBlock = (Element) condition;
                } else if (condition.getNodeName()
        ...
        if (trueBlock == null) {
            ...
            trueBlock = this.getNode()
                .getOwnerDocument()
                .createElement("Then");

            while (this.getNode()
                .hasChildNodes()) {
                trueBlock.appendChild(
                    this.getNode()
                        .getFirstChild());
            }

            this.getNode()
                .appendChild(trueBlock);
        }
