    ...
    protected boolean validateChildren() throws ParserException {
        ...
        if (this._parentDist == null) {
            ...
            if (setting instanceof PDist) {
                ...
                throw new ParserException(
                    "Setting \'" + this.name
                        + "\' in \'"
                        + this._parentDist.toString()
                        + "\' is not supported by this distribution.",
                    this.getNode());
            }
        ...
        if (scaleType != null) {
            if (heightScaledPDist == null) {
                throw new ParserException(
                    "Setting '" + this.name + "' does not support height scaling.",
                    this.getNode());
            } else {
        ...
        if (this._parentDist != null) {
            try {
                this._parentDist
                    .setDistributionSetting(this.name, heightScaledPDist != null ? heightScaledPDist : this.pdist);
            } catch (IllegalAccessException var2) {
                throw new ParserException(
                    "Setting \'" + this.name + "\' in \'" + this._parentDist.toString() + "\' is not configurable.",
                    this.getNode(),
                    var2);
