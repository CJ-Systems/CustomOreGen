...
public class ClientState {
    ...
    public static void onWorldChanged(World currentWorld) {
        _world = currentWorld;
        CustomOreGenBase.log.debug(
            "Client world changed to " + (_world == null ? null
                : _world.getWorldInfo()
                    .getWorldName()));
            ...
            if (geometryData.batchID == _dgBatchID) {
                if (geometryData.dimensionID == _world.provider.dimensionId) {
                    ...
                    if (displayLists != 0) {
                        ...
                        renderer.setPositionTransform(
                            (new Transform())
                                .translate(
                                    (float) (geometryData.chunkX * 16 + 8),
                                    -1.0F,
                                    (float) (geometryData.chunkZ * 16 + 8))
                                .scale(7.5F, 1.0F, 7.5F));
                        ...
                        if (curValue != null && curValue != 0) {
                            ...
                            _chunkDGListBuffer = ByteBuffer.allocateDirect(512)
                            ...
                            if (limit == _chunkDGListBuffer.capacity()) {
                                ...
                                _chunkDGListBuffer = ByteBuffer.allocateDirect(limit * 8)
                                    .order(ByteOrder.nativeOrder())
                                    .asIntBuffer();
                                oldBuffer.rewind();
