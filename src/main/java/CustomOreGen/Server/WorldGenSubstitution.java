...
public class WorldGenSubstitution extends WorldGenerator implements IOreDistribution {
    ...
    @DistributionSetting(name = "minHeight", info = "Minimum substitution height")
    public int minHeight;

    @DistributionSetting(name = "maxHeight", info = "Maximum substitution height")
    public int maxHeight;

    @DistributionSetting(name = "minSurfRelHeight", info = "Minimum surface-relative substitution height")
    public int minSurfRelHeight;

    @DistributionSetting(name = "maxSurfRelHeight", info = "Maximum surface-relative substitution height")
    ...
    public boolean validate() throws IllegalStateException {
        ...
        if (this.additionalRange < 0) {
            this._valid = false;
            throw new IllegalStateException(
                "Invalid additional scan range \'" + this.additionalRange + "\' for " + this);
            ...
            this._valid = false;
            throw new IllegalStateException(
                "Invalid height range [" + this.minSurfRelHeight + "," + this.maxSurfRelHeight + "] for " + this);
        } else {
                        ...
                        for (int x = minX; x < maxX; ++x) {
                            for (int z = minZ; z < maxZ; ++z) {
                                BiomeGenBase biome = chunk
                                    .getBiomeGenForWorldCoords(x, z, world.provider.worldChunkMgr);
                                    ...
                                    for (int y = xzminh; y <= xzmaxh; ++y) {
                                        ...
                                        if (arrangement.matchesAt(world, random, worldX, y, worldZ)) {
                                            ...
                                            if (match != null && matchBlock.canBlockStay(world, worldX, y, worldZ)
                                                ...
                                                ++this.placedBlocks;
                                                TileEntityHelper
                                                    .readFromPartialNBT(world, worldX, y, worldZ, match.getNBT());
    ...
    private boolean isSurfaceBlock(Block block) {
        ...
        return material == Material.clay || material == Material.grass
