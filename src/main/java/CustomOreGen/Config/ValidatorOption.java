...
public class ValidatorOption extends ValidatorNode {
    ...
    protected boolean validateChildren() throws ParserException {
        ...
        if (this._type == ChoiceOption.class) {
            ...
            option = groupName;
            Iterator<ValidatorChoice> defValue = this.validateNamedChildren(2, "Choice", new ValidatorChoice.Factory())
                .iterator();

            while (defValue.hasNext()) {
                ValidatorChoice loadedValueStr = defValue.next();
                groupName
                    .addPossibleValue(loadedValueStr.value, loadedValueStr.displayValue, loadedValueStr.description);
            }
            ...
            if (!groupName1.setLimits(defValue1, err)) {
                throw new ParserException(
                    "Numeric option value range [" + defValue1 + "," + err + "] is invalid.",
                    this.getNode());
            }
            ...
            if (!groupName1.setDisplayLimits(dmin, dmax, dincr)) {
                throw new ParserException(
                    "Numeric option display range/increment [" + dmin + "," + dmax + "]/" + dincr + " is invalid.",
                    this.getNode());
            }
        ...
        option.setDisplayState(
            this.validateNamedAttribute(
                ConfigOption.DisplayState.class,
                "displayState",
                option.getDisplayState(),
                true));
        ...
        if (groupName2 != null) {
            ...
            if (defValue3 == null || !(defValue3 instanceof ConfigOption.DisplayGroup)) {
                throw new ParserException(
                    "Option \'" + groupName2 + "\' is not a recognized Display Group.",
                    this.getNode());
            }
        ...
        Object defValue2 = valueType == null ? null
            : this.validateNamedAttribute(valueType, "default", (Object) null, true);
        ...
        if (this.getParser().target.getConfigOption(option.getName()) != null) {
            ...
            this.getParser().target.getConfigOptions()
                .add(option);
            ...
            if (loadedValueStr1 != null) {
                ...
                CustomOreGenBase.log.warn(
                    "The saved value \'" + loadedValueStr1
                        + "\' for Config Option \'"
                        + ((ConfigOption) option).getName()
                        + "\' is invalid"
                        + err1
                        + ".  "
                        + "The default value \'"
                        + (defValue2 == null ? option.getValue() : defValue2)
                        + "\' will be used instead.");
            }

            if (defValue2 != null && !(option.setValue(defValue2))) {
                throw new ParserException(
                    "Invalid default value \'" + defValue2 + "\' for option \'" + option.getName() + "\'.",
                    this.getNode());
            } else {
                return true;
            }
