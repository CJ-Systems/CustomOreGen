...
public class MapGenClusters extends MapGenOreDistribution {
    ...
    @DistributionSetting(name = "Size", info = "Roughly the number of blocks in every deposit.  No range.")
    public final PDist clSize = new PDist(8.0F, 0.0F);
    @DistributionSetting(name = "Frequency", info = "Number of deposits per 16x16 chunk.  No range.")
    ...
    protected static final DistributionSettingMap _clusterSettingsMap = new DistributionSettingMap(
        MapGenClusters.class);
    ...
    private class ClusterComponent extends Component {
        ...
        public ClusterComponent(StructureGroup structureGroup, float x, float y, float z, Random random) {
            ...
            for (int s = 0; s < this.rad.length; ++s) {
                ...
                super.boundingBox.minX = Math
                    .min(super.boundingBox.minX, MathHelper.floor_float(xCenter - this.rad[s]));
                super.boundingBox.minY = Math
                    .min(super.boundingBox.minY, MathHelper.floor_float(yCenter - this.rad[s]));
                super.boundingBox.minZ = Math
                    .min(super.boundingBox.minZ, MathHelper.floor_float(zCenter - this.rad[s]));
                super.boundingBox.maxX = Math
                    .max(super.boundingBox.maxX, MathHelper.ceiling_float_int(xCenter + this.rad[s]));
                super.boundingBox.maxY = Math
                    .max(super.boundingBox.maxY, MathHelper.ceiling_float_int(yCenter + this.rad[s]));
                super.boundingBox.maxZ = Math
                    .max(super.boundingBox.maxZ, MathHelper.ceiling_float_int(zCenter + this.rad[s]));
            }
        }
