...
public class ConsoleCommands {
    ...
    @CommandDelegate(desc = "Dumps a summary of a distribution or a specific distribution setting to the console.")
    public String cogInfo(@ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") @ArgOptional String distribution,
        @ArgName(name = "setting") @ArgOptional String setting,
        @ArgName(name = "detail") @ArgOptional(defValue = "false") boolean detail) {
        ...
        Collection<IOreDistribution> dists = distribution != null && !distribution.isEmpty()
            ? config.getOreDistributions(distribution)
            : null;
                ...
                if (pattern != null) {
                    ...
                    for (Entry<String, Object> entry : values.entrySet()) {
                        buildFieldValue(
                            ...
                            "  ",
                            (String) entry.getKey(),
                            detail ? (String) descriptions.get(entry.getKey()) : null,
                            entry.getValue());
                    }
                }
            }
    ...
    @CommandDelegate(desc = "Sets the global wireframe rendering mode.  Omit mode to cycle through modes.")
    public void cogWireframeMode(ICommandSender sender,
        @ArgName(name = "None|Polygon|Wireframe|WireframeOverlay") @ArgOptional String renderMode) {
        if (sender instanceof EntityPlayerMP) {
            (new CustomPacketPayload(PayloadType.DebuggingGeometryRenderMode, renderMode))
                .sendToClient((EntityPlayerMP) sender);
        } else {
            throw new IllegalArgumentException(
                "/cogWireframeMode is a client-side command and may only be used by a player.");
        }
    }

    @CommandDelegate(desc = "Clears cached structure information.  Omit distribution name to clear all distributions.")
    public String cogClear(@ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") @ArgOptional(defValue = ".*") String distribution) {
        int count = 0;

        for (IOreDistribution dist : ServerState.getWorldConfig(world)
            .getOreDistributions(distribution)) {
            ++count;
            dist.clear();
            dist.validate();
        }
    ...
    public String cogPopulate(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "chunkRange") int chunkRange,
        @ArgName(name = "distribution") @ArgOptional(defValue = ".*") String distribution,
        @ArgName(name = "centerX") @ArgOptional Integer centerX,
        @ArgName(name = "centerZ") @ArgOptional Integer centerZ) {
        ...
        return "Populated " + list.size()
            + " distributions in "
            + (2 * chunkRange + 1) * (2 * chunkRange + 1)
            + " chunk(s) around ("
            + cX
            + ",0,"
            + cZ
    ...
    @CommandDelegate(desc = "Sets the parent distribution.  Omit parent name to clear parent distribution.")
    public String cogParent(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "parent") @ArgOptional String parent) {
        WorldConfig cfg = ServerState.getWorldConfig(world);
        ...
        if (parent != null) {
            ...
            if (count.size() > 1) {
                throw new IllegalArgumentException(
                    "Parent name \'" + parent + "\' is ambiguous (matches " + count.size() + " distributions).");
            }

            parentDist = (IOreDistribution) count.iterator()
                .next();
        }
    ...
    private static int changeBiomeDescriptor(String settingName, ICommandSender sender, WorldServer world,
        String distribution, String descriptor, float weight, boolean clear) {
        int count = 0;
        ...
        for (IOreDistribution dist : ServerState.getWorldConfig(world)
            ...
            try {
                ...
                if (ex == null) {
                    throw new IllegalArgumentException(
                        "Distribution \'" + dist + "\' does not support descriptor " + settingName + ".");
                }

                if (!(ex instanceof BiomeDescriptor)) {
                    throw new IllegalArgumentException(
                        "Setting " + settingName + " on Distribution \'" + dist + "\' is not a biome descriptor.");
                }
    ...
    private static int changeBlockDescriptor(String settingName, ICommandSender sender, WorldServer world,
        String distribution, String descriptor, float weight, boolean clear, boolean describesOre, boolean matchFirst,
        boolean isRegexp, String nbt) {
        int count = 0;
        ...
        for (IOreDistribution dist : ServerState.getWorldConfig(world)
            ...
            try {
                ...
                if (!(ex instanceof BlockDescriptor)) {
                    throw new IllegalArgumentException(
                        "Setting " + settingName + " on Distribution \'" + dist + "\' is not a block descriptor.");
                }
                ...
                if (ex instanceof BlockDescriptor) {
                    ...
                    desc.add(
                        ...
                        isRegexp,
                        matchFirst,
                        nbt == null ? null : (NBTTagCompound) nbtBase);
                    ++count;
                }
    ...
    @CommandDelegate(desc = "Adds an ore block.")
    public String cogAddOreBlock(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "block") String block,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight,
        @ArgName(name = "matchFirstOre") @ArgOptional(defValue = "false") boolean matchFirstOre,
        @ArgName(name = "nbt") @ArgOptional String nbt) {
        String setting = IOreDistribution.StandardSettings.OreBlock.name();
        int count = changeBlockDescriptor(
            setting,
            sender,
            world,
            distribution,
    ...
    @CommandDelegate(desc = "Sets the ore block (clearing any previous ore blocks).")
    public String cogSetOreBlock(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "block") String block,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight,
        @ArgName(name = "matchFirstOre") @ArgOptional(defValue = "false") boolean matchFirstOre,
        @ArgName(name = "nbt") @ArgOptional String nbt) {
    ...
    @CommandDelegate(desc = "Adds a replaceable block.")
    public String cogAddReplaceable(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "block") String block,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight,
        @ArgName(name = "replacesOre") @ArgOptional boolean replacesOre,
        @ArgName(name = "isRegexp") @ArgOptional boolean isRegexp) {
    ...
    @CommandDelegate(desc = "Sets the replaceable block (clearing any previous replaceable blocks).")
    public String cogSetReplaceable(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "block") String block,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight,
        @ArgName(name = "replacesOre") @ArgOptional boolean replacesOre,
        @ArgName(name = "isRegexp") @ArgOptional boolean isRegexp) {
    ...
    @CommandDelegate(desc = "Adds a target biome.")
    public String cogAddBiome(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "biome") String biome,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight) {
    ...
    @CommandDelegate(desc = "Sets the target biome (clearing any previous biomes).")
    public String cogSetBiome(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "biome") String biome,
        @ArgName(name = "weight") @ArgOptional(defValue = "1") float weight) {
        String setting = IOreDistribution.StandardSettings.TargetBiome.name();
    ...
    public String cogSetting(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "setting") String setting,
        @ArgName(name = "average") float average, @ArgName(name = "range") @ArgOptional(defValue = "0") float range,
        @ArgName(name = "type") @ArgOptional(defValue = "uniform") Type type) {
        int count = 0;
    ...
    public String cogSettingEx(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "distribution") String distribution, @ArgName(name = "setting") String setting,
        @ArgName(name = "value") String value) {
        int count = 0;
    ...
    @CommandDelegate(desc = "Dumps a summary of an Option to the console.")
    public String cogOptionInfo(@ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "option") @ArgOptional String option,
        @ArgName(name = "detail") @ArgOptional(defValue = "false") boolean detail) {
        StringBuilder msg = new StringBuilder();
        WorldConfig config = ServerState.getWorldConfig(world);
    ...
    public String cogOption(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "option") String option, @ArgName(name = "value") String value) {
        WorldConfig cfg = ServerState.getWorldConfig(world);
        ...
        while (path.hasNext()) {
            ...
            if (opt.setValue(value)) {
                ...
                ConsoleCommand
                    .sendText(sender, "\u00a7cInvalid value \'" + value + "\' for Option \'" + opt.getName() + "\'");
            }
        }
    ...
    @CommandDelegate(isDebugging = false, desc = "Enabled or disable debugging mode for the current dimension.")
    public String cogEnableDebugging(ICommandSender sender, @ArgName(name = "dimension") WorldServer world,
        @ArgName(name = "enable") @ArgOptional(defValue = "true") boolean enabled) {
    ...
    public String cogLoadConfig(@ArgName(name = "dimension") WorldServer world) {
        ServerState.clearWorldConfig(world);
