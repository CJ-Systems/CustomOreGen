...
public class ValidatorNode {
        ...
        if (this._validatorMap != null) {
            ...
            for (Node child : childList) {
                List<Object> key = Arrays.asList(
                    new Object[] { child.getNodeType(), child.getNodeName()
                        .toLowerCase() });
    ...
    protected void checkChildrenValid() throws ParserException {
        ...
        for (int i = 0; children != null && i < children.getLength(); ++i) {
            ...
            switch (child.getNodeType()) {
                case Node.TEXT_NODE:
                    if (child.getNodeValue() == null || child.getNodeValue()
                        .trim()
                        .isEmpty()) {
                        break;
                    }
    ...
    protected final <T extends ValidatorNode> LinkedList<T> validateNamedChildren(int nodeTypeMask, String nodeName,
        IValidatorFactory<T> factory) throws ParserException {
        ...
        for (int i = -attrCount; i < childCount; ++i) {
            ...
            if ((nodeTypeMask >>> validator.getNodeType()) % 2 != 0 && (nodeName == null || validator.getNodeName()
    ...
    protected final <T> T validateNamedAttribute(Class<? extends T> attrType, String attrName, T defaultValue,
        boolean allowElements) throws ParserException {
        ...
        LinkedList<ValidatorSimpleNode> children = this
            .validateNamedChildren(mask, attrName, new ValidatorSimpleNode.Factory(attrType, evaluator));
    ...
    protected final <T> T validateRequiredAttribute(Class<T> attrType, String attrName, boolean allowElements)
                        ...
                        if (newNode.getNodeType() == 2) {
                            if (parent.getNodeType() != 1) {
                                throw new ParserException(
                                    "Attempting to merge attribute to non-element node.",
                                    newNode);
                            }
