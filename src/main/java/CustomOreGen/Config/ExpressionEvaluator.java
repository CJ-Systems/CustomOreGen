...
public class ExpressionEvaluator {
        ...
        if (input.isEmpty()) {
            ...
            try {
                ...
                throw new EvaluatorException(
                    ...
                    new Token(expression, expression.length(), expression.length()));
            }
            ...
            switch (token.type) {
                ...
                case NOT_EQUAL:
                    ...
                    if (lhs1 instanceof Number && rhs1 instanceof Number) {
                        ...
                        if (!lhs1.getClass()
                            .isAssignableFrom(rhs1.getClass())) {
                            throw new EvaluatorException(
                                "Cannot compare equality of \'" + lhs1.getClass()
                                    .getSimpleName()
                                    + "\' and \'"
                                    + rhs1.getClass()
                                        .getSimpleName()
    ...
    public Stack<Token> parse(String expression) throws EvaluatorException {
        ...
        for (heldTop = this.getToken(expression, 0); heldTop
            != null; heldTop = this.getToken(expression, heldTop.end)) {
            ...
            if (assoc == Assoc.LEFT != (lastAssoc == Assoc.NONE)) {
                if (heldTop.type == TokenType.ADD) {
                    ...
                    if (heldTop.type != TokenType.SUBTRACT) {
                        throw new EvaluatorException(
                            "\'" + heldTop + "\' cannot follow \'" + lastToken + "\'.",
                            heldTop);
                    }
            ...
            switch (heldTop.type.associativity) {
                ...
                case RIGHT:
                    while (!held.isEmpty()) {
                        ...
                        if (heldTop1.type.associativity != Assoc.LEFT && heldTop1.type.associativity != Assoc.RIGHT
                            || heldTop.type.associativity == Assoc.RIGHT
                                && heldTop.type.precedence == heldTop1.type.precedence
                            || heldTop.type.precedence < heldTop1.type.precedence) {
                            break;
                        }
                ...
                if (Character.isDigit(ch)) {
                    ...
                    try {
                        return isReal
                            ? new Token(
                                ...
                                TokenType.NUMBER,
                                Double.valueOf(Double.parseDouble(input.substring(start, end))))
                            : new Token(input, start, end, TokenType.NUMBER, Long.decode(input.substring(start, end)));
                    ...
                    if (!Character.isLetter(ch) && ch != '_') {
                        if (ch == '"' || ch == '\'') {
                            ...
                            if (end < 0) {
                                throw new EvaluatorException(
                                    "Unmatched string delimeter " + ch + ".",
                                    new Token(input, start, start + 1));
                            }
    ...
    public static enum TokenType {
        ...
        EXPONENT("^", 20, Assoc.LEFT, true,
            new EvaluationDelegate(false, Math.class, "pow", new Class[] { Double.TYPE, Double.TYPE })),
        ...
        ABS("abs", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "abs", new Class[] { Double.TYPE })),
        SIGN("sign", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "signum", new Class[] { Double.TYPE })),
        MIN("min", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "min", new Class[] { Double.TYPE, Double.TYPE })),
        MAX("max", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "max", new Class[] { Double.TYPE, Double.TYPE })),
        FLOOR("floor", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "floor", new Class[] { Double.TYPE })),
        CEILING("ceil", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "ceil", new Class[] { Double.TYPE })),
        ROUND("round", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "round", new Class[] { Double.TYPE })),
        EXP("exp", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "exp", new Class[] { Double.TYPE })),
        LOG("log", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "log", new Class[] { Double.TYPE })),
        SINE("sin", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "sin", new Class[] { Double.TYPE })),
        COSINE("cos", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "cos", new Class[] { Double.TYPE })),
        TANGENT("tan", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "tan", new Class[] { Double.TYPE })),
        ARCSINE("asin", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "asin", new Class[] { Double.TYPE })),
        ARCCOSINE("acos", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "acos", new Class[] { Double.TYPE })),
        ARCTANGENT("atan", 20, Assoc.RIGHT, true,
            new EvaluationDelegate(false, Math.class, "atan", new Class[] { Double.TYPE })),
    ...
    public static class EvaluationDelegate {
        ...
        private EvaluationDelegate(boolean passToken, Object obj, Class<?> clazz, String methodName,
            Class<?>... argTypes) {
            Method method = null;
                    ...
                    if (m.getName()
                        ...
                        if (method != null) {
                            throw new IllegalArgumentException(
                                "Method \'" + methodName + "\' is ambiguous for class " + clazz.getSimpleName());
                        }
            ...
            if (method == null) {
                throw new IllegalArgumentException(
                    "Method \'" + methodName + "\' is not defined for class " + clazz.getSimpleName());
                    ...
                    if (!valid) {
                        throw new IllegalArgumentException(
                            "Method \'" + methodName
                                + "\' for class "
                                + clazz.getSimpleName()
                                + " must take a String or Token as the first parameter");
                    }
                }
        ...
        protected Object evaluate(Token token, Stack<Token> argumentStack)
            throws EmptyStackException, EvaluatorException {
        ...
        public String toString() {
            String prefix = this._method.getDeclaringClass()
                .getSimpleName() + "." + this._method.getName();
    ...
    public static class EvaluatorException extends Exception {
        ...
        public String getMessage() {
            if (this.token == null) {
                ...
                return String.format(
                    "At %s(%d-%d) %s : %s",
                    new Object[] { name, Integer.valueOf(this.token.start), Integer.valueOf(this.token.end), this.token,
                        super.getMessage() });
            }
        }
        ...
        public String toString() {
            ...
            if (this.token != null && this.token.source != null && !this.token.source.isEmpty()) {
                ...
                msg.append(
                    this.token.source.substring(quoteStart, quoteEnd)
                        .replace("\n", " "));
